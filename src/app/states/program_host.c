...
int program_host_enter(void) {
    if (server_is_init() || client_is_init()) return 1;
    name[0]  = '\0';
    name_len = 0;
    return 0;
}
...
void program_host_render(void) {
    ...
    const char *players_text = TextFormat(
        "Players (press up/down): %d", (int)num_players);

    int players_text_width = MeasureText(players_text, font_size);
    float players_cy       = cy + (text_height / 2.f) + 20.f;
    DrawText(players_text,
        cx - (players_text_width / 2.f),
        players_cy,
        font_size,
        WHITE);
