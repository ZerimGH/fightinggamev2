...
void game_frame_init(GameFrame *gf) {
    ...
    uint16_t spacing = (WORLD_W - PLAYER_W) / (gf->num_players - 1);
...
void game_frame_apply_input(GameFrame *gf, uint8_t player_id, Input input) {
    ...
    player_state_update(&gf->players[player_id]);
    player_state_apply_inputs(&gf->players[player_id],
        gf->inputs[player_id],
...
static void player_punch_update(Player *p) {
    if (p->anim_frame >= 3) { player_state_change(p, PSIdle); }
}
...
uint64_t hash_game_frame(GameFrame *gf) {
    if (!gf) { return 0; }
    return hash((void *)&gf->players[0], sizeof(Player) * gf->num_players);
}
